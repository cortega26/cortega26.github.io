...
> `sha256-BZJxfeK/xslBDpYiGCWMd7N9XoSnojTl/uxahqkTpwQ=` = GA4 inline bootstrap (`window.dataLayer`/`gtag` stub, `ga4Id=G-2HK4GHK7GR`) — `BaseLayout.astro:108-114` (`define:vars={{ga4Id}}`). Generado con `openssl dgst -sha256 -binary | openssl base64` sobre el contenido exacto del `<script>` inline tras `astro build`. Sin este hash el stub sería bloqueado (no hay `unsafe-inline` en `script-src`). Ver `plans/006-plausible-to-ga4-migration.md:5`.
...
- The site now loads GA4 directly via `gtag.js` (`https://www.googletagmanager.com` + `https://www.google-analytics.com`/`region1`) — see Plan 006. Inline bootstrap is allowed via `sha256-BZJxfeK/xslBDpYiGCWMd7N9XoSnojTl/uxahqkTpwQ=`.
