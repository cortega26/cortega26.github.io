...
|---|---|---|---|---|---|---|---|---|
| `TS-001` | Conectar GA4 (gtag.js directo, `G-2HK4GHK7GR`) — supersede Plausible | `P1` | `S` | `src/layouts/BaseLayout.astro` (inline stub `define:vars` + `PUBLIC_GA4_MEASUREMENT_ID`), `public/assets/js/track.js` (`tt_location`/`tt_label`/`tt_status` + solo `gtag`) | Reemplazar Plausible por GA4 directo sin GTM: stub síncrono `window.dataLayer/gtag` antes de `async gtag/js`, mapping `ttTrack({location,label,status})→gtag('event',{tt_location,tt_label,tt_status})`, conservar Ahrefs | Click hero CTA aparece como `cta_book_call {tt_location:"hero"}` en GA4 Realtime/DebugView (`/g/collect` 204) | `npm run build` + check `dist/en/index.html` inline stub SHA `BZJxfeK/xslBDpYiGCWMd7N9XoSnojTl/uxahqkTpwQ=` + `curl -sSI` CSP + preview gtag | `Hecho` (2026-08-21, Plan 006) — pendiente aplicar CSP en Cloudflare y crear dimensiones `tt_*` |
